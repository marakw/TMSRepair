...
def ui_check(rel_time, pre, post, options):
...
def ui_select(epochs, options, compnum, freq, fftbins):
    ...
    win = ui_raw(options)
    rel_time = epochs.times*1000
    temp = epochs.tesa['S'][compnum, :, :]

    fig, ((sp1, sp2), (sp3, sp4)) = plt.subplots(2, 2)

    # plot time series in color code (red = suspected artifact)
    if epochs.tesa['compclass'][compnum] != 1:
        colour = 'r'
    else:
        colour = 'b'

    sp1.plot(rel_time, np.mean(temp, 1), colour)
    sp1.title.set_text('Component time series across channels')
    sp1.set_xlabel('Time [ms]')
    sp1.set_ylabel('Amplitude [a.u.]')

    # plot topographical plot
    ch_names = [chan for chan in epochs.ch_names if chan in options['chanpicks']]
    ...
    if options['freqscale'] == 'raw':
        sp3.plot(freq, fftbins)
    elif options['freqscale'] == 'log':
        sp3.plot(freq, np.log(fftbins))
    elif options['freqscale'] == 'log10':
        sp3.plot(freq, np.log10(fftbins))
    elif options['freqscale'] == 'db':
        sp3.plot(freq, 10*np.log10(fftbins))
    ...
    tp1 = np.argmin(np.abs(rel_time - options['plottimex'][0]))
    tp2 = np.argmin(np.abs(rel_time - options['plottimex'][1]))
    ...
    def on_closing():
        ...
        choice = clicked.get()
        epochs.tesa['compclass'][compnum] = int(art_types.index(choice)) + 1
        plt.close('all')
        win.destroy()
    ...
    clicked.set(art_types[int(epochs.tesa['compclass'][compnum])-1])
